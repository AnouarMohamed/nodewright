...
`make create-kind-cluster` renders `config/local-dev/ctlptl-config.yaml` with `KIND_NODE_VERSION` from `operator/deps.mk`, then creates the kind cluster and a local registry at `localhost:5005`. It also runs `make setup-kind-cluster`, which is idempotent and labels the test node and creates the `skyhook` namespace pull secret.
